                ...
                <select name="parent_id" lay-search>
                    ...
                    @if(!empty($categorys))
                        @foreach($categorys as $perm)
                            <option value="{{$perm['id']}}" {{ isset($show->id) && $perm['id'] == $show->parent_id ? 'selected' : '' }} >{{$perm['title']}}</option>
                            @if(isset($perm['_child']))
                                @foreach($perm['_child'] as $childs)
                                    <option value="{{$childs['id']}}" {{ isset($show->id) && $childs['id'] == $show->parent_id ? 'selected' : '' }} >┗━━{{$childs['title']}}</option>
                                    @if(isset($childs['_child']))
                                        @foreach($childs['_child'] as $lastChilds)
                                            <option value="{{$lastChilds['id']}}" {{ isset($show->id) && $lastChilds['id'] == $show->parent_id ? 'selected' : '' }} >┗━━━━{{$lastChilds['title']}}</option>
                                        @endforeach
                                    @endif
                                @endforeach
                            @endif
