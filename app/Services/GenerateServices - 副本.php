...
class GenerateServices
{
     ...
     * @return [type]
     */
    public static function parsePicture($data, $is_password = false)
    {
        ...
        for($i = 0; $i < 13; $i++)
        {
            ...
            $img_url = $dir. DIRECTORY_SEPARATOR .'a_'.$i.'.jpg';

            self::$editor->open( $image, $filename );
            self::$editor->crop( $image, 30, 30, 'top-left', $w, 10);

            $filter = Grafika::createFilter('Sobel');
            self::$editor->apply( $image, $filter );

            self::$editor->save( $image, $img_url );

            $tmp = [];
            foreach(self::$code as $k => $item)
            {
                $local = 'code' .DIRECTORY_SEPARATOR . $item;

                if(self::run($local, $img_url))
                {
                    $result[$i] = $k;
                }
            }

            self::$editor->free( $image );
        }

dd(1);
        $response['country']        = implode('', array_slice($result, 0, 2)); // 2位
        $response['factory']        = implode('', array_slice($result, 2, 2)); // 2位
        $response['category']       = implode('', array_slice($result, 4, 3)); // 3位
        $response['check_code']     = implode('', array_slice($result, 7, 6)); // 6位
        ...
        {
            $w = $i * 40 ?: 10;
            $img_url = $dir. DIRECTORY_SEPARATOR .'b_'.$i.'.jpg';

            self::$editor->open( $image, $filename );
            self::$editor->crop( $image, 30, 30, 'top-left', $w, 50);
            self::$editor->save( $image, $img_url );
        ...
        $response['child_category'] = implode('', array_slice($result, 0, 2)); // 2位
        $response['id']             = intval(implode('', array_slice($result, 2, 11))); // 11位
        ...
        for($i = 0; $i < 12; $i++)
        {
            ...
            $img_url = $dir. DIRECTORY_SEPARATOR .'c_'.$i.'.jpg';

            self::$editor->open( $image, $filename );
            self::$editor->crop( $image, 30, 30, 'top-left', $w, 90);
            self::$editor->save( $image, $img_url);
        ...
        $response['time_code']      = implode('', array_slice($result, 0, 12)); // 12位
        ...
        if($is_password)
        {
            ...
            for($i = 0; $i < 12; $i++)
            {
                ...
                self::$editor->open( $image, $filename );
                self::$editor->crop( $image, 30, 30, 'top-left', $w, 130);
                self::$editor->save( $image, $img_url);
            ...
            $response['password']      = implode('', array_slice($result, 0, 12)); // 12位
        }
    ...
    public static function saveTmpPicture($data, $dir)
    {
        try
        {
            ...
            $s = file_put_contents($filename, $data);
